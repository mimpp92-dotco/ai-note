...
- `whisper/server.py` — `127.0.0.1:8123` 바인딩(`LOCAL_STT_HOST`/`LOCAL_STT_PORT`). health + fixed-ID transcribe. Client path를 받지 않고 `AI_NOTE_DATA_ROOT/meetings/{meetingId}` 아래만 파생한다.
...
- **Dispatch claim**: meeting별 `.whisper-dispatch.json`은 Whisper만 쓴다. `(meetingId,dispatchId)`가 protocol identity이며 audio hash, `accepted|segments_published|raw_published`, `pending|durable|best_effort`를 기록한다. Same audio의 fresh proposal은 canonical ID adoption을 반환하고 audio mismatch는 fail-closed한다.
