...
function failureResponse(
  ...
  if (result.reason === "invalid_transcript" || result.reason === "invalid_summary") {
    return publicErrorResponse("invalid_request", 400, { field: result.field ?? "transcript" });
  }
...
export async function PATCH(
  ...
  let body: unknown;
  try {
    body = await parseBoundedJsonBody(request, 2 * 1024 * 1024);
  } catch (error) {
    return requestBodyErrorResponse(error);
  }
  const parsed = manualTranscriptRequestSchema.safeParse(body);
  if (!parsed.success) {
    return publicErrorResponse("invalid_request", 400, { field: "transcript" });
  }

  const result = await saveManualTranscript({ id, ...parsed.data });
  return result.ok
