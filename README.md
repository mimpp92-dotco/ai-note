...
Run this from a fresh clone with Node.js ≥ 20. Before changing `node_modules` or `.next`, bootstrap checks this repository's runtime ownership. If no runtime exists, it runs the prerequisite doctor, `HUSKY=0 npm ci`, a secret-free build, and the owned background app/Whisper supervisor on free bounded loopback ports. If an owned runtime already exists, bootstrap does not stop, restart, install, or build; it verifies and opens that runtime and reports that this install/update was not applied. To apply it, first confirm that no recording is in progress, run `npm run app:stop`, then rerun the same `--launch` command. Stale, invalid, unsafe, or unverifiable runtime state fails closed without signaling a process or mutating the build.

Both a new and reused runtime must pass four bounded readiness checks: app health, Whisper health, an AI NOTE-identified root HTML response, and the existing `/api/library` public response. Library modes `ready`, `degraded_last_good`, and `degraded_fallback` are supported; corrupt, unsupported-version, and I/O failures are not repaired or overwritten by bootstrap. Only after all four pass does it print and open `AI_NOTE_URL=http://localhost:<actual-port>`. Bootstrap never writes library data directly; the existing app API remains its owner. It never attaches to or stops an existing process on ports 3000/8123, and it does not create or overwrite `.env.local`.

If the doctor reports a missing prerequisite, follow its visible OS-specific instruction and rerun the same command. A Whisper health response that explicitly reports missing `ffmpeg` stops startup immediately with the same static installation action instead of waiting for the general timeout. Bootstrap does not silently use `sudo`, install OS packages, log in to a provider, pull an Ollama model, or download a Whisper model. A summarizer is optional for recording and local transcription; choose and verify one later in **Settings → 요약 모델**.

On Windows, `Path`/`PATH` and `PATHEXT` are interpreted case-insensitively for both the doctor and child processes; duplicate values are not concatenated. The exact `uv.exe` resolved by the doctor environment is used for Whisper, including paths with spaces. After installing a prerequisite or the standalone Codex CLI, open a new PowerShell so its `PATH` is current. If an owned AI NOTE runtime was already running, confirm that no recording is active, stop it with `npm run app:stop`, and rerun `node scripts/bootstrap.mjs --launch` so the new environment is inherited.
...
In **Settings → 요약 모델**, Claude CLI offers its default, Sonnet, Opus, Haiku, or a custom model; Codex CLI offers its default or a custom model; and Ollama discovers locally installed models with refresh and custom-input fallback. Saving immediately checks the persisted configuration. On Windows, the Codex desktop app package is not a substitute for the standalone Codex CLI; the doctor warns only when that WindowsApps package is clearly the first `PATH` candidate. A successful Claude/Codex check confirms only that the CLI binary can run; authentication and actual generation are confirmed by the first summary. A missing or unhealthy summarizer never blocks bootstrap, recording, or local transcription.
