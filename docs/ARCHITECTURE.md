...
Windows merge job은 Node 22의 `npm ci`, setup/bootstrap/Codex 주입식 regression, secret 없는 build, pinned Chromium install·doctor 뒤 기존 `e2e/smoke.spec.ts`만 세 viewport에서 실행한다. OS temp source snapshot 이름에는 의도적으로 공백이 있다. 이 job은 install/build와 synthetic first-use browser hydration의 조합이지 실제 bootstrap end-to-end가 아니며 mic, user data, `uv`/`ffmpeg`, Whisper model/service, Codex login, local provider, external browser network를 사용하지 않는다.

## End-user bootstrap·owned runtime

- Clone 뒤 canonical command는 Node stdlib만 필요한 `node scripts/bootstrap.mjs --launch`다. 이 command는 install/build보다 repository-local runtime ownership을 먼저 판정한다. `absent`일 때만 `scripts/setup.mjs` doctor → `HUSKY=0 npm ci` → `npm run build` → background supervisor를 수행한다. `owned`이면 기존 runtime만 검증·개방하고 자동 stop/restart/install/build하지 않으며 update 미적용을 알린다. 변경 적용은 사용자가 녹음이 없는지 확인한 뒤 `npm run app:stop`과 같은 `--launch`를 명시적으로 다시 실행하는 절차다. Stale/invalid/unsafe/unverifiable state는 signal·build mutation 없이 fail-closed한다.
- Doctor의 Node/`uv`/`ffmpeg` failure는 안전한 OS별 조치와 같은 command 재실행을 안내한다. Windows child environment는 `Path`/`PATH`·`PATHEXT`를 case-insensitive key 하나로 정규화하며 inherited duplicate를 deterministic하게 고르고 explicit override가 우선한다. 값을 합치지 않는다. Doctor의 `which`와 spawn이 이 environment를 공유하고 resolved exact `uv.exe`를 argv로 넘기며 path 공백에도 `shell:false`를 유지한다. Absolute uv path는 runtime state나 설정에 저장하지 않는다.
...
- 새 runtime과 reused owned runtime은 `canonicalAppUrl()`의 `http://localhost:<actual-app-port>` authority에서 app health, same-origin `/api/whisper/health`, AI NOTE title로 식별되는 bounded root HTML, existing `/api/library` public response를 모두 probe한다. Library mode `ready|degraded_last_good|degraded_fallback`만 지원하며 corrupt/unsupported/I/O를 bootstrap이 repair·overwrite하지 않는다. Library write는 app-api owner만 수행한다. 네 surface가 모두 ready일 때만 `AI_NOTE_URL`을 출력하고 `app:status`도 app/whisper/root/library 결과를 구분한다. 실패는 raw body/path/PID/token/provider output 없는 고정 오류로 낮춘다.
- Whisper health가 정확한 current ffmpeg-missing public 상태이면 generic startup timeout 전에 static OS별 설치와 `--launch` 재실행을 안내한다. Ready 전 interrupt/error handler는 이번 supervisor가 확보한 child handle만 idempotent cleanup하며 global PID/port scan이나 ownership 불명 signal을 하지 않는다. Supported desktop opener도 URL을 shell interpolation 없는 argv로 전달한다. Headless/unsupported/opener failure는 server 성공을 유지하고 exact URL과 agent browser surface fallback을 출력한다.
...
- Codex는 optional summarizer다. Windows doctor는 첫 PATH 후보가 명확한 `%ProgramFiles%\WindowsApps\OpenAI.Codex_*` package일 때만 desktop package 혼동을 warning한다. Windows `EACCES|EPERM|ENOENT` health는 독립 Codex CLI/PATH, 새 PowerShell, 녹음 확인 뒤 owned runtime stop/relaunch 조치를 안내한다. Warning·미설정·health failure는 bootstrap/녹음/전사를 막지 않으며 `codex --version` success는 binary 감지일 뿐 인증·generation 보장이 아니다.
- Bootstrap module import는 side-effect free이며 process/network/port/browser/time/fs boundary를 주입할 수 있다. Unit test는 fake와 temp directory만 사용해 `npm ci`, build, long-lived server, browser opener, external network, model download를 실행하지 않는다. Bootstrap이 `sudo`, package manager, provider login, Ollama pull 또는 Whisper model download를 몰래 실행하지 않는다. Foreground `npm run dev`는 contributor lifecycle로 분리한다. 설치 target/agent handoff 결정은 ADR [0023](decisions/0023-installation-and-first-run-ux.md)을 따른다.
