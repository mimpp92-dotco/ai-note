...
2. **배치 전사** — 녹음 종료 시 로컬 whisper가 전체 오디오를 한 번에 전사한다. 선택지는 `large-v3`(품질 우선 기본)와 `large-v3-turbo`(더 빠른 후보)로 고정하며 세그먼트 타임스탬프를 포함한다. 설정 저장과 model prepare/download는 분리하고, 새 dispatch가 acceptance 시 effective model을 내구 snapshot한다.
3. **최초 교정 + 요약** — 전사가 끝나면 앱의 백그라운드 워커가 로컬 CLI(claude/codex)나 Ollama로 최초 자동 전사 원문을 오타·문단 교정한 전체 스크립트와, 그 스크립트에 근거한 구조화 요약을 차례로 만든다. 기본은 full-context 품질 모드이며 사용자가 명시한 fast mode만 결정적 chunk·provider별 제한 병렬을 사용한다. 이 결합 파이프라인은 최초 생성에만 사용하며 외부 API 키는 필요 없다(구독/로컬 모델, 비용 $0).
...
11. **설치·첫 실행** — Clone 뒤 `node scripts/bootstrap.mjs --launch` 하나가 전제 진단, `HUSKY=0 npm ci`, build, bounded dynamic loopback port의 owned background app/Whisper, health, 실제 URL browser open을 조율한다. 첫 화면은 요약 모델 미설정/불가를 recorder 앞에서 비차단으로 안내하고 **AI 요약 설정 / 요약 없이 회의 녹음**을 제공한다. Provider별 native model selector와 저장 직후 persisted health, 전사·교정 fixed selector와 별도 model prepare를 제공하며 요약 모델이나 optional 내 정보가 없어도 녹음·로컬 전사·일반 검색은 가능하다.
12. **실패 복구·완료 기본 보기** — `retry_transcription`은 목록·상세·저장 결과에 지속 표시하고 exact meeting ID로 기존 durable transcribe API를 다시 호출한다. 교정 성공 뒤 summary 실패·중단은 hidden durable checkpoint를 보존하며 same-key manual retry가 교정 model call을 생략한다. 명시적 실패는 network 복귀·app restart·worker poll로 새 attempt를 만들지 않고 사용자가 retry를 접수할 때만 다시 시작한다. 완료 회의는 explicit query가 없고 usable summary가 있으면 **회의록 요약**을 기본 tab으로 연다.
13. **격리 pipeline benchmark** — 사용자가 exact meeting ID와 실제 local data/provider 사용을 명시적으로 승인한 command만 source meeting을 read-only로 비교한다. `.ai-note-runtime/benchmarks/` private snapshot에서 `large-v3`/Turbo와 full/fast를 실행해 hash·stage wall time·사람 검수 template를 남기며 canonical artifact/status/library를 바꾸지 않는다. 2×/30% threshold를 계산하되 사람 품질 검수가 비어 있으면 추천은 `undecided`다.
...
- 명시적 실패 뒤 auto retry/resume, network listener/watchdog, supervisor restart/autostart, browser pipeline-stage progress UI
- 실제 benchmark와 사람 품질 검수 전 `large-v3-turbo` 또는 fast mode를 권장/default로 승격

### v2로 명시적 연기
chunk-append 전사 크래시 복구 + 디코드 게이트 · 전체 상태 FSM + stale-job 워치독 · **summary map-reduce** · auto-queue-on-green · 세그먼트 클릭 재생 · 무음 워치독/장치 선택/일시정지 · capture_id 재스캔 idempotency. Fast correction chunking은 구현됐지만 summary map-reduce와 자동 queue는 계속 연기한다.
...
- **실패는 수동 재시도**: 최초 dispatch는 자동이지만 한 번 명시적으로 실패한 전사/교정/요약은 자동 재연결·재시작하지 않는다. 교정 checkpoint와 completed fast chunk는 exact key에서만 수동 retry가 재사용하며 한 chunk 실패 뒤 남은 새 chunk나 full fallback을 실행하지 않는다.
- **구조화 LLM 경계**: Summary caller만 generated-summary strict JSON Schema를 adapter에 전달하고 chat은 generic JSON envelope를 유지한다. CLI capability는 generation 전 bounded help probe로만 고정하며 unsupported optional flag를 generation 실패로 알아낸 뒤 같은 prompt를 재실행하지 않는다. Tolerant JSON salvage와 final schema validation은 유지하고 call당 timeout은 30분이다.
