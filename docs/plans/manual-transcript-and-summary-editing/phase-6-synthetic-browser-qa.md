...
앞선 phase가 커밋한 synthetic fixture와 Playwright 시나리오를 실행한다. 최초 verify-only attempt에서 CommonJS 기본 package의 TypeScript Playwright spec이 ESM `.mjs` fixture를 정적으로 import해 loader가 실패했고, 첫 repair 뒤 두 Playwright spec의 exact `모든 회의` assertion이 현재 workspace-qualified H1과 불일치함을 드러냈다. 두 spec repair 뒤에는 세 viewport 모두 browser back이 dirty-edit 확인창보다 먼저 Next navigation을 커밋하는 실제 R8 결함을 재현했다. 이 amended phase는 보존된 두 spec patch와 `RecorderSessionProvider`의 browser-back interception만 TDD로 복구한다. fixture 값은 복제하지 않고 committed fixture를 runtime에 읽으며, 수정 뒤 전체 3-viewport suite와 screenshot·assertion·console manifest를 runner-owned Git local journal에서 검증한다. Chrome DevTools MCP나 기존 Chrome session은 이 완료 gate에 관여하지 않는다.
...
- 허용된 두 Playwright spec과 RecorderSessionProvider source/test 밖 수정이나 fixture 값 복제가 필요하면 중단한다.
...
2. `--strategy continue`가 blocked attempt의 두 spec patch를 적용한 상태에서 `npm run test:e2e`를 실행해, browser back이 dirty-edit 확인창보다 먼저 목록 navigation을 커밋하는 현재 R8 실패를 확인한다. 보존 patch가 없으면 static ESM import와 workspace-qualified H1 실패부터 순서대로 복구한다.
3. 허용된 두 spec의 module-loading과 accessible heading repair를 그대로 유지한다.
...
4. `src/components/__tests__/RecorderSessionProvider.test.tsx`에 browser-back event ordering 회귀를 먼저 추가해 RED를 만든 뒤 `src/components/RecorderSessionProvider.tsx`를 최소 수정한다.
   - dirty content 또는 unsaved audio blocker가 활성화된 native popstate는 Next navigation이 destination을 렌더하기 전에 현재 URL을 복원하고 확인창을 연다.
   - 취소는 현재 route와 draft를 보존하고, 명시적 discard는 원래 destination을 정확히 한 번만 커밋한다.
   - blocker가 없거나 명시적 discard 뒤 재개된 popstate는 정상 navigation을 방해하지 않는다.
   - event listener는 provider lifecycle에 맞춰 제거하며 영구적인 global suppression이나 새 dependency를 추가하지 않는다.
5. `npm run test:e2e`가 OS temp에 allowlist source snapshot을 만들고 pinned Chromium을 시작한다.
...
6. smoke와 manual editing spec을 desktop-1440, mobile-390, mobile-320 세 project에서 모두 실행한다.
...
7. automatic evidence fixture와 reporter가 다음을 manifest에 기록한다.
...
8. runner가 manifest schema·artifact hash·viewport·synthetic provenance를 직접 검증한다. 명령 종료 뒤 app child와 temp snapshot이 정리되어야 한다.
9. phase 전후 Git scope를 비교한다. 허용된 네 파일 외 tracked/untracked 제품 경로 변화가 하나라도 있으면 browser assertion이 통과했더라도 phase를 실패시킨다. verify command 자체는 제품 파일을 수정하지 않아야 한다.
...
- 중간 RED: runtime import repair 뒤 같은 명령이 두 spec의 stale exact `모든 회의` H1 assertion으로 실패한다.
- 현재 RED: 두 spec repair 뒤 `RecorderSessionProvider.test.tsx`의 event-ordering 회귀와 전체 E2E가 browser-back 선행 navigation 때문에 실패한다.
- GREEN: provider 단위 테스트와 typecheck가 통과하고, spec이 committed `.mjs` fixture와 workspace-qualified H1을 검증하며 전체 E2E가 세 viewport에서 통과해야 한다.
...
- 이 phase에서는 repository 문서를 수정하지 않는다. 허용된 두 spec의 harness compatibility와 provider browser-back guard repair만 커밋한다.
...
```bash
npm test -- src/components/__tests__/RecorderSessionProvider.test.tsx
npm run typecheck
npm run test:e2e
```
