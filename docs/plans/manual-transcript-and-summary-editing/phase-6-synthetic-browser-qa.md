...
앞선 phase가 커밋한 synthetic fixture와 Playwright 시나리오를 실행한다. 최초 verify-only attempt에서 CommonJS 기본 package의 TypeScript Playwright spec이 ESM `.mjs` fixture를 정적으로 import해 loader가 실패했으므로, 이 amended phase는 해당 spec 한 파일의 module-loading 경계만 TDD로 복구한다. fixture 값은 복제하지 않고 committed fixture를 runtime에 읽으며, 수정 뒤 전체 3-viewport suite와 screenshot·assertion·console manifest를 runner-owned Git local journal에서 검증한다. Chrome DevTools MCP나 기존 Chrome session은 이 완료 gate에 관여하지 않는다.
...
- Playwright spec 한 파일 밖 수정이나 fixture 값 복제가 필요하면 중단한다.
...
2. fresh 모델이 phase baseline의 `npm run test:e2e`를 먼저 실행해 `Cannot use import statement outside a module` 실패를 RED evidence로 재현한다. 다른 실패라면 원인을 추측해 범위를 넓히지 않고 중단한다.
3. `e2e/manual-transcript-and-summary-editing.spec.ts`만 수정해 CommonJS로 transform되는 TypeScript spec이 ESM `.mjs` fixture를 Node/Playwright가 지원하는 runtime 경계로 읽게 한다.
...
8. phase 전후 Git scope를 비교한다. 허용된 spec 한 파일 외 tracked/untracked 제품 경로 변화가 하나라도 있으면 browser assertion이 통과했더라도 phase를 실패시킨다. verify command 자체는 제품 파일을 수정하지 않아야 한다.

## 테스트 (먼저 작성)

- RED: baseline의 `npm run test:e2e`가 TypeScript spec의 static ESM fixture import에서 `Cannot use import statement outside a module`로 실패해야 한다.
- GREEN: spec이 committed `.mjs` fixture를 지원되는 runtime module boundary로 읽고 같은 명령이 세 viewport에서 통과해야 한다.
...
- 이 phase에서는 repository 문서를 수정하지 않는다. 허용된 spec의 module-loading repair만 커밋한다.
