...
- Phase 5가 synthetic fixture와 feature spec을 제품 변경과 함께 커밋한다. Phase 6 browser 실행은 static ESM import, workspace-qualified H1 assertion, Next popstate보다 늦게 동작하는 dirty-edit guard를 순서대로 드러냈다. amended Phase 6은 두 spec의 harness compatibility와 `RecorderSessionProvider`의 browser-back interception만 TDD로 복구한 뒤 browser suite 전체를 실행한다.
